...
bool Scene10::init() {
    ...
    this->scheduleOnce(schedule_selector(Scene10::textAppears), 2);
    this->scheduleOnce(schedule_selector(Scene10::gameButtonCallBack), 5);
...
void Scene10::gameButtonCallBack(float dt){
    ...
    clipboard->setScale(0.3*visibleSize.width / clipboard->getContentSize().width,
                        0.35*visibleSize.height / clipboard->getContentSize().width);
...
void Scene10::textAppears(float dt)
{
    
    //// get text from json script object
    
    auto textString1 = Json_getString(script, "10.1", "Go Gators!");
    auto textString2 = Json_getString(script, "10.2", "Go Gators!");
