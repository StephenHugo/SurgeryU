...
void FrameNav::addFrameNav(Scene* layer) {
...
void FrameNav::tapScreen(cocos2d::Scene *chapterLayer)
{
...
void FrameNav::toggleBorder(int tag, Scene *frameLayer) {
	frameLayer->removeChildByTag(tag);
}

void FrameNav::toggleIconBar(int tag, Scene* frameLayer) {
	frameLayer->removeChildByTag(tag);
}
...
void FrameNav::createIconBar(Scene* frameLayer) {
...
void FrameNav::toggleSound(cocos2d::Ref* pSender, cocos2d::Scene *chapterLayer)
{
    
    auto vol = 0.7f;
    //*(CocosDenshion::SimpleAudioEngine::getInstance()->getBackgroundMusicVolume()/VOLUMEMUSIC)+0.15f;
    ...
    listener->onTouchEnded = [volumeControl](cocos2d::Touch *touch, cocos2d::Event *event){
        ...
        auto vol = 0.7f; //CocosDenshion::SimpleAudioEngine::getInstance()->getBackgroundMusicVolume()/VOLUMEMUSIC;
        auto volEffects = 0.7f; //CocosDenshion::SimpleAudioEngine::getInstance()->getEffectsVolume();
...
cocos2d::Label* FrameNav::createText(const char* textString, cocos2d::Scene* chapterLayer) {

	Size visibleSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();

	auto label = Label::createWithTTF(textString, FELTFONT, visibleSize.height * TEXTSIZE);

	label->setPosition(Vec2(visibleSize.width * 0.5 + origin.x, visibleSize.height * 0.14 + origin.y));
	...
	label->setLineHeight(1.9 * visibleSize.height * TEXTSIZE);
	label->setOpacity(0);
        ...
        || !sameChapter)
    {
        ...
        switch (currentChapter) {
            case CHLAUNCH:
                ...
                AudioEngine::preload(MUSICDH);
                AudioEngine::play2d(MUSICDH, 1, 0.7f);
                
                break;
                
            case CHWAIT:
                AudioEngine::preload(MUSICWAIT);
                AudioEngine::play2d(MUSICWAIT, 1, 0.7f);
                break;
                
            case CHPREOP:
                AudioEngine::preload(MUSICPREOP);
                AudioEngine::play2d(MUSICPREOP, 1, 0.7f);
                break;
                
            case CHOR:
                AudioEngine::preload(MUSICOR);
                AudioEngine::play2d(MUSICOR, 1, 0.7f);
                break;
                
            case CHPOSTOP:
                AudioEngine::preload(MUSICPOSTOP);
                AudioEngine::play2d(MUSICPOSTOP, 1, 0.7f);
                break;
                
            default:
                AudioEngine::preload(MUSICLAUNCH);
                AudioEngine::play2d(MUSICLAUNCH, 1, 0.7f);
                break;
        }
    }
